...
`docs/specs/document-generation.md`; the one-minute rule summary in
`docs/business_processes/document-generation.md`. This file is the
developer map.
...
| `sheets/formula-validator.ts`       | Hand-rolled formula checker (Decision 11 — `hyperformula` is GPLv3, excluded): a security blocklist (WEBSERVICE/HYPERLINK/EXEC/…/DDE-pipe) that downgrades a hit to literal text, plus an advisory A1/range reference checker. No evaluation, ever                                                                                                                                                                                                                                                                                              |
...
| `document-export.service.ts`        | The only Angular piece: hydration, image decryption, filename derivation, delivery via `saveBlob`                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
2. **`<cog-doc>` blocks** — the model emits the document inside its reply
   ...
   message content, never stored artifacts**.
...
The facade seam is the point (spec Decision 5):
...
- **No GPLv3/copyleft dependencies** (spec Decision 11).

// Types for the `<cog-doc>` document source contract (see
// docs/business_processes/document-generation.md). No Angular imports — this module is
// parsed on every streamed delta and must stay framework-free and pure.
import { z } from 'zod';

// Spec: source block (tags included) must fit the existing 1 MB
// `messages.data` column with sealing overhead and the rest of the reply.
export const COG_DOC_MAX_SOURCE_BYTES = 256 * 1024;

/**
 * cogDocSpecSchema validates the `spec='{…}'` JSON attribute (spec).
 ...
 * what fail-open display shows verbatim (spec,).
 */
...
export type MessageSegment =
  { kind: 'markdown'; text: string } | { kind: 'document'; block: CogDocBlock };

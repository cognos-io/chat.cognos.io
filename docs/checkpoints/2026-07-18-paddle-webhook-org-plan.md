...
| ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `TestOrgPaddleWebhookActivatesOrgSubscription`               | Org `subscription.created` with `custom_data.org_id` creates `org_billing`, sets `seat_quantity = 1`, activates creator membership. | Create `organisations` row, post `subscription.created` with `custom_data.org_id`, `items[0].quantity = 1`. | `org_billing.plan_type = "payg"`, `org_billing.seat_quantity = 1`, `org_billing.paddle_subscription_id = "sub_org_1"`, creator membership `is_active = true`. | `organisations.paddle_customer_id` persisted; no `user_billing` touched.                             |
| `TestOrgPaddleWebhookSeatQuantitySync`                       | `subscription.updated` with new item quantity updates `org_billing.seat_quantity` to match Paddle.                                  | Org activated with qty 1, post update with `items[0].quantity = 3`.                                         | `org_billing.seat_quantity = 3`.                                                                                                                              | Quantity reflects active seats; `pending_seat_quantity` unchanged if no pending decrement.           |
...
| `TestOrgPaddleWebhookSeatRemoveNextCycle`                    | `pending_seat_quantity` decrements at rollover, not mid-cycle.                                                                      | Org with `seat_quantity = 3`, `pending_seat_quantity = 2` (one member offboarded), post rollover.           | After rollover: `seat_quantity = 2`, `pending_seat_quantity = null`.                                                                                          | Cycle close uses old `seat_quantity = 3` for floor calculation; next cycle uses 2.                   |

...
| SR-004 | P2       | Rotating the MFA encryption key without a keyring would strand existing TOTP seeds.                  | Application / ops        | Closed 22 Jul 2026 | Versioned keyring (`mfa.totp_encryption_key` + optional `totp_encryption_key_previous`); lazy re-seal on TOTP verify; rotation runbook in [`mfa-login.md`](./business_processes/mfa-login.md).           | Before rotating the production MFA key          |

...
Why store FX and provider cost separately from `amount_rappen`: every
figure on the ledger must be independently auditable from the others. With
all three present, anyone can re-derive: `provider × (1 + margin) × fx ≈
user_cost`, and `balance_after = balance_before + amount` for trial.

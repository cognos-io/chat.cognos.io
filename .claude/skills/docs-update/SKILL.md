...
uv run --script .claude/skills/docs-update/scripts/docs_tool.py apply en /tmp/patch.json
```
...
uv run --script ../.claude/skills/docs-update/scripts/docs_tool.py check   # parity + figure coverage
```
...
- `check` — locale parity, articles with no figure, missing figure files, localised-shot counts.
  Exit non-zero on parity drift.

...
Everything needed to build the Cognos UI packages. `@cognos/ui` is the
framework-agnostic foundation package. Tokens are the source of truth in
**`tokens.json`** (machine-readable) and **`tokens.css`** (drop-in CSS
variables). This doc explains how to apply them and specs each component.
Lineage: **Atlassian Design System** conventions with an **Emerald** brand
accent.
...
├─ tokens.css         # default stylesheet entrypoint (tokens + themes)
├─ app.css            # optional app-level globals
├─ styles/
│  ├─ tokens.css      # foundation tokens only
│  └─ themes.css      # semantic light/dark + accent tokens
└─ theme.js           # tiny framework-agnostic helpers for theme attributes

@cognos/ui-angular
├─ src/
│  ├─ primitives/     # Button, IconButton, Lozenge, Toggle, TextField, Avatar
│  ├─ overlays/       # Menu, Sheet, Modal
│  ├─ navigation/     # Nav, NavItem, Drawer, Breadcrumbs
│  └─ chat/           # Composer, UserMessage, AssistantMessage, SectionMessage
└─ public-api.ts
```

- `@cognos/ui` should stay **framework-agnostic**. It owns tokens, CSS custom
  properties, theme attribute conventions, and tiny DOM helpers only.
- Framework packages such as `@cognos/ui-angular` should consume `@cognos/ui`
  and provide the actual component implementations.
- Components should stay theme-agnostic — read `--cog-*` only.
- Generate platform tokens from `tokens.json` via **Style Dictionary** if you
  also need iOS/Android.
- `tokens.css` is the default required import. `app.css` is opt-in for
  application shells and should not be imported automatically by component
  packages.
...
```ts
import { applyTheme } from '@cognos/ui';
```

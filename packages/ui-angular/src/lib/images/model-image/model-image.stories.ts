...
const meta: Meta<StoryArgs> = {
  ...
  render: (args) => ({
    props: args,
    template: `<cog-model-image [src]="src" [prompt]="prompt" [state]="state" [tag]="tag" [tone]="tone" [host]="host" [width]="width" (open)="open($event)" (download)="download($event)" (regenerate)="regenerate($event)" (variations)="variations($event)" (saveToVault)="saveToVault($event)" />`,
  }),
};
...
export const OnPrem: Story = { args: { tag: "ON-PREM", tone: "green", host: "On-prem" } };
export const ThisDevice: Story = { args: { tag: "THIS DEVICE", tone: "purple", host: "this device" } };

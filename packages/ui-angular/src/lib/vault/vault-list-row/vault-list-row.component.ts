...
@Component({
  ...
  template: `
    <div class="cog-vault-list-row" [class.cog-vault-list-row--top]="top()" role="button" tabindex="0" (click)="open.emit(file())" (keydown.enter)="open.emit(file())" (keydown.space)="$event.preventDefault(); open.emit(file())">
      @if (file().kind === 'image' && file().img) {
        <span class="cog-vault-list-row__thumb"><img class="cog-vault-list-row__thumb-image" [src]="file().img!" alt="" /></span>
      } @else {
      ...
      <div class="cog-vault-list-row__copy">
        ...
        <div class="cog-vault-list-row__details">{{ file().size }} · {{ file().meta }} · {{ file().when }}</div>
      </div>

      <span class="cog-vault-list-row__refs" [class.cog-vault-list-row__refs--linked]="file().refs > 0">
        <cog-icon name="link" [size]="12" [tone]="file().refs > 0 ? 'link' : 'text-subtlest'" />
        <span>{{ file().refs > 0 ? file().refs + ' chats' : '—' }}</span>
      </span>
